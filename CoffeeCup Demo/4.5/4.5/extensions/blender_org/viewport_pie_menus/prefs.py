...
def update_prefs_on_file(self=None, context=None) -> tuple[str, dict]:
    ...
    if not prefs:
        print("3D Viewport Pie Menus: Couldn't save preferences because the Preferences class was already unregistered.")
        return
...
class PrefsFileSaveLoadMixin:
    ...
    def register_autoload_from_file(delay=0.0):
        def timer_func(_scene=None):
            ...
            areas = 'Window', 'Object Mode', '3D View'
            if not all(i in kc.user.keymaps for i in areas):
...
def get_bug_report_url(stack_trace=""):
    ...
    return (
        "https://projects.blender.org/extensions/space_view3d_pie_menus/issues/new?template=.gitea/issue_template/bug.md&field:body="
