...
class add_mesh_bolt(Operator, AddObjectHelper):
    ...
    def execute(self, context):
        ...
        base_merge_dist_mm = 0.0001
        dynamic_merge_dist = base_merge_dist_mm * adjusted_scale
        obj = None  # Will hold the created or modified object

        if bpy.context.mode == "OBJECT":
            # Common mesh creation for OBJECT mode
            mesh_data_block = createMesh.Create_New_Mesh(self, context, adjusted_scale)
            
            bm = bmesh.new()
            bm.from_mesh(mesh_data_block)
            # Apply remove_doubles once for OBJECT mode operations
            bmesh.ops.remove_doubles(bm, verts=bm.verts, dist=dynamic_merge_dist)

            if (context.selected_objects and context.active_object and
                    context.active_object.data is not None and
                    'Bolt' in context.active_object.data.keys() and self.change):
                # Modifying an existing object
                obj = context.active_object
                use_smooth = False  # Default
                if obj.data.polygons:  # Check if polygons exist before accessing
                    use_smooth = bool(obj.data.polygons[0].use_smooth)

                bm.to_mesh(obj.data)  # Write cleaned BMesh data to existing object's mesh
                # bm.free() is called after if/else block

                # Preserve flat/smooth choice
                if use_smooth:
                    bpy.ops.object.shade_smooth()
                else:
                    bpy.ops.object.shade_flat()
                
                # The original mesh_data_block was temporary and its data copied to obj.data (via bmesh)
                # So, remove the temporary mesh_data_block
                bpy.data.meshes.remove(mesh_data_block)

                try:
                    # Ensure active object is selected if ops.object context is needed
                    # For vertex_group_remove, it typically operates on the active object
                    bpy.ops.object.vertex_group_remove(all=True)
                except RuntimeError:  # Catch if no vertex groups or other op error
                    pass
            else:
                # Creating a new object
                # Write the cleaned BMesh data back to the mesh_data_block
                bm.to_mesh(mesh_data_block)
                # bm.free() is called after if/else block
                
                obj = object_utils.object_data_add(context, mesh_data_block, operator=self)
                # mesh_data_block is now used by 'obj', so it's not removed here.
            
            bm.free()  # Free the BMesh object

            # Common properties for the object (either new or existing, if obj was set)
            if obj:
                obj.data["Bolt"] = True
                obj.data["change"] = False  # Set to False for new, or reset for existing after change
                for prm in BoltParameters():
                    if hasattr(self, prm):  # Ensure the property exists on self
                        prop_value = getattr(self, prm)
                        try:
                            obj.data[prm] = prop_value
                        except TypeError:
                            # Handle potential type errors if a complex type can't be assigned directly
                            # For example, convert to string or handle specific types as needed
                            print(f"Warning: Could not set obj.data['{prm}'] with value {prop_value} of type {type(prop_value)}")

        elif bpy.context.mode == "EDIT_MESH":  # Changed from 'if' to 'elif'
            obj = context.edit_object  # This is the object being edited
            # Create new mesh geometry to be added
            mesh_to_add = createMesh.Create_New_Mesh(self, context, adjusted_scale)

            bm = bmesh.from_edit_mesh(obj.data)  # Get BMesh from the object being edited
            bm.from_mesh(mesh_to_add)            # Append the new geometry to the edit-mode BMesh
            
            # Apply remove_doubles after appending, for EDIT_MESH mode
            # This will merge vertices from the original mesh and the newly added mesh where appropriate
            bmesh.ops.remove_doubles(bm, verts=bm.verts, dist=dynamic_merge_dist)
            
            bmesh.update_edit_mesh(obj.data)  # Write changes back to the edit-mode mesh
            
            # Remove the temporary mesh_to_add datablock as its geometry is now in the edit BMesh
            bpy.data.meshes.remove(mesh_to_add)
            # obj.data properties are usually not set here as we are modifying existing geometry in edit mode,
            # unless the operator's intent is also to update these based on 'self' properties.
            # If they need to be updated, ensure 'obj' is correctly referenced:
            # obj.data["change"] = True # Or some other relevant status for an edit op
